...
float calculatePointShadow(Light light, vec3 fragPos, int lightIndex);
...
void main()
{
    ...
    for (int i=0; i<numLights; i++)
    {
		...
		{
			if(currLight.type == 1) //Direct light
			{
				shadow = calculateDirectionalShadow(currLight, fragPos, directLightIndex);
			}
			else if(currLight.type == 2)
			{
				shadow = calculatePointShadow(currLight, fragPos, pointLightIndex);
			}
        ...
        illumination += shadow*max(_illumination,0.0f);
	...
	illumination = pow(illumination, vec3(1.0/0.9));
...
vec3 calcRadiance(Material material, Light light, vec3 normal, vec3 fragPos, vec3 viewDir){
	...
	return (kD*diffuse/PI* incomingRadiance + specular)  * nLDot; //diffuse is the material color
}
...
float calculatePointShadow(Light currLight, vec3 fragPos, int lightIndex)
{
	float shadow = 0.0f;
	...
	float offset = 0.1f;
	//Check Center fragment first. If its outside
